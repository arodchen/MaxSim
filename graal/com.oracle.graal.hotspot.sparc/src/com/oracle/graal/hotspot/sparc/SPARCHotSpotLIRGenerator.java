...
public class SPARCHotSpotLIRGenerator extends SPARCLIRGenerator implements HotSpotLIRGenerator {
    ...
    /**
     * The slot reserved for storing the original return address when a frame is marked for deoptimization. The return
     * address slot in the callee is overwritten with the address of a deoptimization stub.
     */
...
// append(new AMD64TailcallOp(args, address));
    ...
    protected void emitDirectCall(DirectCallTargetNode callTarget, Value result, Value[] parameters, Value[] temps, LIRFrameState callState) {
        ...
        if (invokeKind == InvokeKind.Interface || invokeKind == InvokeKind.Virtual) {
            append(new SPARCHotspotDirectVirtualCallOp((ResolvedJavaMethod) callTarget.target(), result, parameters, temps, callState, invokeKind));
        } else {
            ...
            append(new SPARCHotspotDirectStaticCallOp((ResolvedJavaMethod) callTarget.target(), result, parameters, temps, callState, invokeKind, metaspaceMethod));
        }
    }
